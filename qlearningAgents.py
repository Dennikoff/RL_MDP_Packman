...
class QLearningAgent(ReinforcementAgent):
    ...
    def setQValue(self, state, action, value):
        self.QVualues[(state, action)] = value
    ...
    def getQValue(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        tempVal = self.QVualues.get((state, action))
        if tempVal == None:
            return 0.0
        return tempVal
    ...
    def computeValueFromQValues(self, state):
        ...
        for action in actionList:
            ...
            if (maxVal == None) or (tempVal > maxVal):
                maxVal = tempVal
        return maxVal
    ...
    def update(self, state, action, nextState, reward):
        ...
        newValue = (1 - self.alpha) * currentValue + self.alpha * sample
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for featureKey in featureList.keys():
            sumValue += featureList[featureKey] * self.weights[featureKey]
        return sumValue
    ...
    def update(self, state, action, nextState, reward):
        ...
        for featureKey in featureList.keys():
            self.weights[featureKey] += difference * self.alpha * featureList[featureKey]
    ...
    def final(self, state):
        ...
        if self.episodesSoFar == self.numTraining:
            ...
            # print('FINAL: ', self.featExtractor.getFeatures(state, action)['eats_ghosts'])
            pass
